...
restaurant_query_prompt = PromptTemplate.from_template(
    """
As of {current_time}, fetch up to 20 candidate rows from Restaurant
where the 'address' column contains the selected district '{gu}':
...
accommodation_query_prompt = PromptTemplate.from_template(
    """
As of {current_time}, fetch up to 20 candidate rows from Accommodation
where the 'address' column contains the selected district '{gu}':
...
아래 사용자 city, district, travel_theme, companions, group_size, breakfast, lunch, dinner, Question, SQLQuery, SQLResult 를 참고해 **한국어**로 사용자 친화적인 추천을 작성하세요.
특히, 사용자가 지정한 변수들에 걸맞게 답변을 시작하세요.
...
- 예상 이동 수단 및 소요 시간.
- 아침식사, 관광지, 점심식사, 관광지, 저녁식사, 숙소 순으로 아래 형식처럼 모두 작성해줘.
- 장소를 이동할 때, 대중교통이나 자차, 도보를 통해 몇 분 이동해야 하고, 거리는 어떤지 까지 포함해서 간략하지만, 정확하게 작성해줘.
- 블로그 리뷰, 사용자 리뷰가 많을 경우 -> 인기가 많다는 등의 투로 작성해줘.

원하는 형식:

<간략한 브리핑> [아침 식사] -> [관광지] -> [점심 식사] -> [관광지] -> [저녁 식사] -> [숙소]

1. 아침식사
  **식당명**
  - 종류: 일식
  - 이유: 예(평점이 제일 높다, 블로그 리뷰 수가 제일 많다, 사용자 리뷰 수가 제일 많다, 인증된 식당이다)
  - 이동: 약 Y분

2. 관광지
  **관광지명**
  - 종류: 산
  - 이유: 예(사용자가 산을 원하기 때문에 추천)
  - 이동: 약 Y분
...
Question: {question}
SQLQuery_Restaurant: {query_restaurant}
SQLQuery_Accommodation: {query_accommodation}
SQLQuery_TourInfo: {query_tourinfo}
SQLResult_Restaurant: {fetch_db_restaurant}
SQLResult_Accommodation: {fetch_db_accommodation}
SQLResult_TourInfo: {fetch_db_tourinfo}
Answer:
"""
)
