...
def load_and_process_location_data():
    ...
    shapefile_path = "/home/aitomic123/TourAgent/data/raw/서울시 주요 120장소 영역.shp"
    gdf = gpd.read_file(shapefile_path)

    # 중심점 계산
    gdf['centroid_x'] = gdf.geometry.centroid.x # gdf에 중심점 x,y 좌표 추가
    gdf['centroid_y'] = gdf.geometry.centroid.y
    gdf = gdf.to_crs(epsg=4326)
    
    # 2. 기상청 격자 데이터 로드
    region = pd.read_excel("/home/aitomic123/TourAgent/data/raw/기상청41_단기예보 조회서비스_오픈API활용가이드_격자_위경도.xlsx")
    ...
    # 기상청 단기예보 조회서비스 격자_위경도
    seoul_region_gdf = seoul_region_gdf.rename(columns={
        "경도(초/100)": "lon", # lon -> map_x -> lon가 맞음 (merged_ef를 바꿔줘야 함)
        "위도(초/100)": "lat" # lat -> map_y -> lat가 맞음 (merged_df를 바꿔줘야 함)
    })
    
    # 3. 관광지 목록 로드 및 병합
    tour_list = pd.read_excel('/home/aitomic123/TourAgent/data/raw/서울시 주요 120장소 목록.xlsx')
